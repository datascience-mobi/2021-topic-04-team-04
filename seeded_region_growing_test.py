...
# calculated mean value of region of newly labeled pixel
def one_region_mean2(img, regions,
                     new_pixel):  # img is array of intensity values, regions is array with region numbers, new_pixel is position of last added pixel
    ...
    for i in Ne:
        ...
        for j in nei:
            if regions[j] != 0:  # only neighboring pixels which are sorted
                distance.append(np.abs((img[i] - means[int(regions[
                                                               j] - 1)])) / max_intensity)  # saves tuple of normalized distance of pixel to neighbor j and region of j
                ...
                if regions[j] == regions[new_pixel]:  # calculates distance only for pixel adjacent to updated region
                    ...
                    for k in nei:
                        if regions[k] != 0:  # compare all neighboring regions
                            distance.append(np.abs((img[i] - means[int(regions[
                                                                           k] - 1)])) / max_intensity)  # saves tuple of normalized distance of pixel to neighbor j and region of j
                            region_number.append(regions[k])
                    min_dist = min(distance)  # saves minimal distance to 1 of its neighbors in distance array
                    # only calculates new distances once
                    pos_min_dist = distance.index(min_dist)
                    nearest_reg[i] = region_number[
                        int(pos_min_dist)]  # saves number of nearest region # saves number of nearest region
...
def label(regions, distances, nearest_reg,
          Ne):  # regions is array of region numbers, distances is array of distances, nearest_reg is array of nearest region number
